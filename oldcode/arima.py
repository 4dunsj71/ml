...
with StepwiseContext(max_dur=30):
    with StepwiseContext(max_steps=10):
        model = auto_arima(y_train,d = d, start_p=1, start_q=1, m= m,D = 1 ,method = 'nm', stepwise=True, trace=True, seasonal = True)
    #model = auto_arima(y_train,d = d, stepwise=False, trace=True, seasonal = True)
